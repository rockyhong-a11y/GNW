...
정보 구성은 **인벤 발매 캘린더**(https://www.inven.co.kr/webzine/calendar/)를
참조하여 재구성했습니다.
...
[공개 소스]                          [큐레이션]
 ...
 RSS(인벤    ─┤─ build-data.mjs ◀──────  CBT/OBT/사전예약 등
  발매 캘린더) │   (정규화·중복제거·병합)   API에 없는 국내 정보)
            ─┘        │
                      ▼
...
### 🔙 참조 사이트 롤백
참조처를 "인벤 발매 캘린더 단일"로 바꾸기 전(멀티소스) 상태는
`data/curated.multisource.backup.json` 에 보존되어 있습니다. 결과가 마음에 들지 않으면:

```bash
node scripts/rollback-sources.mjs   # 백업 복원 + games.json 재생성
# 또는
